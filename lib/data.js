...
export const social = {
  contactmail: "tecknologia99@gmail.com",
};
...
export const filters = {
  ...
  Brand: {
    ...
    options: {
      ...
      honor: { name: "Honor", operation: (p) => p?.brand == "Honor" },
      nubia: { name: "Nubia", operation: (p) => p?.brand == "Nubia" },
      blackshark: {
        name: "Black Shark",
  ...
  Display: {
    ...
    options: {
      ...
      small: {
        name: "Small Display",
        operation: (p) =>
          (p?.deviceType == "Smartphone" && p?.display[0]?.size <= 6.4) ||
      ...
      medium: {
        ...
        operation: (p) =>
          (p?.deviceType == "Smartphone" &&
            p?.display[0]?.size >= 6.4 &&
            p?.display[0]?.size <= 6.7) ||
          (p?.deviceType == "Tablet" &&
...
export function getBrandKey(brand) {
  if (!brand) return null;

  return (
    Object.entries(filters.Brand.options).find(
      ([, value]) => value.name.toLowerCase() === brand.toLowerCase(),
    )?.[0] || null
  );
}
